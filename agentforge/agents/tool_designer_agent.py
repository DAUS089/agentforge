"""
Tool Designer Agent for agentforge.
...
class ToolDesignerAgent:
    ...
    def generate_tool_code(self, requirement: ToolRequirement) -> GeneratedTool:
        ...
        # Determine file path
        file_path = f"/tmp/agentforge_custom_tools/{requirement.name.lower()}_tool.py"
...
from agentforge.tools.registry import ToolRegistry, ToolBase

class {req.name}Wrapper(ToolBase):
    """Wrapper to make {req.name} compatible with agentforge registry."""
...
def register_{req.name.lower()}():
    """Register the {req.name} with agentforge."""
