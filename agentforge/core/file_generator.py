"""
File-based crew generation system for agentforge.
...
Generated by agentforge on {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}
"""
...
def main():
    ...
    if len(sys.argv) > 1:
        ...
        print(f"[CHAR] Task input: {{task_input}}")
    else:
        task_input = "{spec.task}"
        print(f"[INFO][CHAR]  No command line arguments provided, using default task")
    
    # Initialize and run the crew
    print(f"[CHAR] Initializing {self._to_class_name(spec.name)} crew...")
            ...
            if 'context' in task_config:
                for context_task_name in task_config['context']:
                    if context_task_name in tasks:
                        ...
                        print(f"[CHAR] Context linked: {{task_name}} <- {{context_task_name}}")
                    else:
                        print(f"[CHAR][CHAR]  Context task '{{context_task_name}}' not found for task '{{task_name}}'")
            
            # Update task with context
            if context_tasks:
                tasks[task_name].context = context_tasks
                print(f"[CHAR] Task '{{task_name}}' has {{len(context_tasks)}} context task(s)")
            else:
                print(f"[CHAR] Task '{{task_name}}' has no context (root task)")
    ...
    def run(self, task_input: str = None) -> str:
        ...
        try:
            ...
            if task_input and task_input.strip():
                print(f"\\n[CHAR] Task Input Received: {{task_input}}")
                ...
                if 'main_task' in self.tasks:
                    ...
                    print(f"[CHAR] Updated main task description with input")
                else:
                    print("[CHAR][CHAR]  No main_task found to update")
            else:
                print("[INFO][CHAR]  No specific task input provided, using default task description")
            
            # Execute the crew
            print("\\n[CHAR] Starting crew execution...")
            result = self.crew.kickoff()
            return str(result)
            
        except Exception as e:
            error_msg = f"Crew execution failed: {{str(e)}}"
            print(f"[CHAR] {{error_msg}}")
            return error_msg
...
This project was generated by agentforge on {datetime.now().strftime("%Y-%m-%d %H:%M:%S")}.
