...
class OllamaProvider(LLMProvider):
    ...
    def get_llm_config(self, config) -> Dict[str, Any]:
        return {
            "model": config.llm.model or "llama3.1",
            "base_url": config.llm.ollama_host or "http://localhost:11434",
...
class LlamaCppProvider(LLMProvider):
    ...
    def get_llm_config(self, config) -> Dict[str, Any]:
        return {
            "model": config.llm.model or "llama-3.1-8b",
            "model_path": config.llm.model_path,
            "n_ctx": config.llm.n_ctx or 2048,
            "n_gpu_layers": config.llm.n_gpu_layers or 0,
            "temperature": config.llm.temperature or 0.7,
            "max_tokens": config.llm.max_tokens or 2000
        }
    
    def validate_config(self, config) -> bool:
        # LlamaCpp requires model_path
        return bool(config.llm.model_path)
...
def get_llm_config_for_crewai(config) -> Dict[str, Any]:
    ...
    if provider == "anthropic" and not model.startswith("anthropic/"):
        ...
        # Ollama uses ollama/ prefix for litellm
        base_config["model"] = f"ollama/{model}"
        base_config["api_base"] = base_config.get("base_url")
    elif provider == "llamacpp":
        # LlamaCpp uses direct model names
        base_config["model"] = model
        # Add LlamaCpp specific parameters
        if "model_path" in base_config:
            base_config["model_path"] = base_config["model_path"]
        if "n_ctx" in base_config:
            base_config["n_ctx"] = base_config["n_ctx"]
        if "n_gpu_layers" in base_config:
            base_config["n_gpu_layers"] = base_config["n_gpu_layers"]
