...
class LLMConfig(BaseModel):
    """LLM configuration."""
    provider: str = "openai"  # openai, google, anthropic, deepseek, ollama, llamacpp, custom
    model: str = "gpt-4"
    ...
    ollama_host: Optional[str] = "http://localhost:11434"  # Ollama server URL
    
    # LlamaCpp specific parameters
    model_path: Optional[str] = None  # Path to model file
    n_ctx: Optional[int] = 2048       # Context window size
    n_gpu_layers: Optional[int] = 0   # Number of layers to offload to GPU
...
class Config:
    """Configuration manager for agentforge."""
    ...
    def _get_default_config_path(self) -> str:
        ...
        # First check for .agentforge/config.yaml in current directory
        local_config = Path(".agentforge/config.yaml")
    ...
    def _load_config(self) -> agentforgeConfig:
        """Load configuration from file or create default."""
        if os.path.exists(self.config_path):
            try:
                with open(self.config_path, 'r') as f:
                    config_data = yaml.safe_load(f) or {}
                return agentforgeConfig(**config_data)
    ...
    def save_config(self, config: Optional[agentforgeConfig] = None):
    ...
    def update_from_env(self):
        """Update configuration from environment variables. (DISABLED - Use .agentforge/config.yaml only)"""
        # Environment variable override is disabled to force users to use .agentforge/config.yaml
